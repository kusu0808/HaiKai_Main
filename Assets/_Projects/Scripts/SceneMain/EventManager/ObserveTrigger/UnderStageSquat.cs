...
namespace Main.EventManager
{
    public sealed partial class EventManager
    {
        private async UniTaskVoid UnderStageSquat(CancellationToken ct)
        {
            while (true)
            {
                int i1 = await WaitUntilConditionReturnMinusOne(() => _borders.UnderStageSquat.Elements.IsInInAny(_player.Position), ct);
                if (i1 is -1) return;
                ...
                int i2 = await WaitUntilConditionReturnMinusOne(() => _borders.UnderStageSquat.Elements.IsInOutAny(_player.Position), ct);
                if (i2 is -1) return;
        ...
        /// </summary>
        private async UniTask<int> WaitUntilConditionReturnMinusOne(Func<int> condition, CancellationToken ct)
        {
